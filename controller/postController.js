...
exports.get_all_posts = async (req, res, next) => {
    try {
        ...
        const posts = await prisma.posts.findMany({        // Repeated func
            where: {
                "id": {in: userIds}
            }
        })
...
exports.get_following_posts = async (req, res, next) => {
    try {
        ...
        const followerIds = followings.map(following => following.followingId);
...
exports.post_post = async (req, res, next) => {
    try {
        ...
        if (!title && !url) return res.status("400").json("Title and File is required")
        ...
        return res.json("Post Published")
